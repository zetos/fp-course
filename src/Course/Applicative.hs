...
instance Applicative ExactlyOne where
  ...
  (<*>) ::
    ...
    -> ExactlyOne a
    -> ExactlyOne b
  (<*>) eoab eoa = let f = runExactlyOne eoab
                       a = runExactlyOne eoa
                    in ExactlyOne (f a)
...
lift3 ::
  ...
  -> k c
  -> k d
lift3 f fa fb fc = let fbcd = f <$> fa <*> fb
                   in fbcd <*> fc
...
lift4 ::
  ...
  -> k d
  -> k e
lift4 f fa fb fc fd = let fabcd = f <$> fa <*> fb <*> fc
                      in fabcd <*> fd
...
replicateA ::
  ...
  -> k a
  -> k (List a)
replicateA a fa = sequence (replicate a fa)
...
filtering ::
  ...
  -> List a
  -> k (List a)
filtering p = foldRight (\a fxs -> lift2 (f a) (p a) fxs) (pure Nil)
  where f a b xs' = if b then a :. xs' else xs'
-- filtering p = foldRight (\a fxs -> lift2 (\b xs' -> if b then a :. xs' else xs') (p a) fxs) (pure Nil)
