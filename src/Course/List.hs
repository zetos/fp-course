...
differenceR :: List P.Float -> P.Float
differenceR = foldRight (-) 0

differenceL :: List P.Float -> P.Float
differenceL = foldLeft (-) 0
...
flatMap ::
  ...
  -> List a
  -> List b
-- flatMap fn = foldRight (\x xs -> fn x ++ xs) Nil
flatMap fn = foldRight ((++) . fn) Nil
-- flatMap fn xs = flatten (map fn xs)
-- flatMap fn = flatten . map fn
...
-- seqOptional = foldRight (\x xs -> _f) (Full Nil)
...
reverse ::
  List a
  -> List a
-- reverse = foldRight (\x acc -> acc :. x) Nil
